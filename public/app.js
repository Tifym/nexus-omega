...
export default async function handler(req, res) {
  try {
    ...
    // Fetch state, but handle the error gracefully if the row is missing
    let { data: state, error: stateErr } = await supabase.from('trading_state').select('*').eq('id', 'main').single();
    
    // Auto-initialize the database row if it doesn't exist!
    if (!state || stateErr) {
        await supabase.from('trading_state').insert({ id: 'main', balance: 10000, initial_balance: 10000 });
        state = { balance: 10000, initial_balance: 10000, total_trades: 0, winning_trades: 0, max_drawdown: 0, last_trade_time: null };
    }
    ...
    res.status(200).json({
      ...
      signal: { text: signal.signal, confidence: signal.confidence },
